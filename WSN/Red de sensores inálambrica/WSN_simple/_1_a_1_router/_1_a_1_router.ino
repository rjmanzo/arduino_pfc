...
void loop() {
  ...
  if ( now - last_time_sent >= interval ) {
    ...
    if ( this_node > 00 || to == 00 ) {                   // Normal nodes send a 'T' ping
      ...
      //payload_t payload_volt = {voltage, {2017, 12, 12, 12, 12, 12}, NODE_ADDRESS, 6}; // sending voltage measure sample
      ok = send_T(to, payload_volt);
      delay(50);
      
      if(sensorline != 2){ //ask for bmp183

        float pressure, bmp_temp;
        bmp183 (pressure, bmp_temp);
        payload_t payload_pres = {pressure, {2017, 12, 12, 12, 12, 12}, NODE_ADDRESS, 4}; // sending voltage measure sample
...
/*void DS3231_update(int[6] t){
  DateTime dt(t[0],t[1],t[2],t[3],t[4],t[5]);
  Wire.begin();
  rtc.begin();
