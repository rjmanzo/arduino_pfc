...
byte node_config[12] = { 0, 1, 1, 0, 0, 15, 0, 0, 0, 0, 0, 0}; // Init flags for dht22 motes
//byte node_config[12] = { 0, 0, 0, 1, 1, 15, 0, 0, 0, 0, 0, 0}; // Init flags for bmp183 motes
...
void loop() {
  /*
  if (EEPROM.read(6) == 0 && EEPROM.read(0)!= 0){ //after first boot, if we dont recive any start sending package we divide sleeptime by 2
    int half_time = EEPROM.read(5) / 2;
  ...
  //chequear el resto del modulo - Si el valor entre del resto es menor a 1 se obtiene un valor de resto = 1. La funcion modulo no devuelve flotante, por ende redondea pero para arriba
  int resto = 4%2;
  ...
  if(EEPROM.read(0)){ //check the setting flags 
    ...
    int sleep_minutes = EEPROM.read(5); //get the time from the flag
...
void sleep_mode(int sleep_minutes) { // nrfl24l01 radio Init
  ...
  */  
  if (sleep_minutes != 0){  // if sleep_minutes is zero the Pro Mini dont sleep
