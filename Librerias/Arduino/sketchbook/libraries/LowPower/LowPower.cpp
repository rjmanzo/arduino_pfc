...
void LowPowerClass::powerDown_off(period_t period, adc_t adc,bod_t bod, timer2_t timer2,
	timer1_t timer1, timer0_t timer0,
	spi_t spi, usart0_t usart0,    twi_t twi)
	{
		// Temporary clock source variable
		...
		if (timer0 == TIMER0_OFF)    power_timer0_disable();
		if (spi == SPI_OFF)            power_spi_disable();
		//NO DEBERÍA DE APAGAR EL USB. DSP NO VA A FUNCIONAR EL USB A TRANSMITIR UN NUEVO SKETCH (REVISAR!!!)
		//if (usart0 == USART0_OFF)    power_usart0_disable();
		if (twi == TWI_OFF)            power_twi_disable();
		...
		if (bod == BOD_OFF)    lowPowerBodOff(SLEEP_MODE_PWR_DOWN);
		{
			#if defined __AVR_ATmega328P__
			lowPowerBodOff(SLEEP_MODE_PWR_DOWN);
			#else
		...
		if (timer0 == TIMER0_OFF)    power_timer0_enable();
		if (spi == SPI_OFF)            power_spi_enable();
		//SINO SE DESACTIVA TAMPOCO ES NECESARIO PRENDERLO!
		//if (usart0 == USART0_OFF)    power_usart0_enable();
		if (twi == TWI_OFF)            power_twi_enable();

		//powerDown_on(ADC_ON,BOD_ON,TIMER2_ON,TIMER1_ON,TIMER0_ON,SPI_ON,USART0_ON,TWI_ON); 
	}

void  LowPowerClass::powerDown_on(adc_t adc,bod_t bod, timer2_t timer2,
	timer1_t timer1, timer0_t timer0,
	spi_t spi, usart0_t usart0,    twi_t twi)
	{
