...
void PowerSaver::sleepInterruptSetup()
{
//Seteo el sleep mode en powerdown mode
SMCR = (1<<SE) | (1<<SM1);
//seteo el sleep mode en 8 sg. Activar en 1 las bits WDP3 Y WDP0 del registro WDTCSR
cli();		// Disable global interrupts
asm("wdr");		// Reset Watchdog Timer
MCUSR = 0; // clear reset flags
WDTCSR |= (1<<WDCE) | (1<<WDE);		// Write logical one to WDCE and WDE
//WDTCSR = (1<<WDE) | (1<<WDP3) | (1<<WDP0);	// Turn on WDT on System Reset Mode - set prescale at 8.0s
//WDTCSR = (1<<WDIE) | (1<<WDP2) | (1<<WDP0);	// Turn on WDT interrupt - set prescale at 0.5s
WDTCSR = (1<<WDIE) | (1<<WDP3) | (1<<WDP0);	// Turn on WDT interrupt - set prescale at 8.0s
sei();	// Enable global interrupts
}

  //****************************************************************

/*void PowerSaver::sleepModeSetup()
{
  //Seteo el sleep mode en powerdown mode
  SMCR = (1<<SE) | (1<<SM1);
  //seteo el sleep mode en 8 sg. Activar en 1 las bits WDP3 Y WDP0 del registro WDTCSR
...
{
  PORTB |= ((1<<DDB5) | (1<<DDB4) | (1<<DDB3) | (1<<DDB2) | (1<<DDB1) | (1<<DDB0));  // set SPI pins (13, 12, 11, 10, 9, 8) to HIGH
  DDRB &= ~((1<<DDB5) | (1<<DDB4) | (1<<DDB3) | (1<<DDB2) | (1<<DDB1) | (1<<DDB0));  // set SPI pins (13, 12, 11, 10, 9, 8) to INPUT
