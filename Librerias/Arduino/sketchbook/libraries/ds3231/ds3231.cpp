...
void DS3231_set (const char* date, const char* time) {
    // sample input: date = "Dec 26 2009", time = "12:34:56"
	uint8_t i, century;
    uint8_t year_s = conv2d(date + 9);
    // Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec 
	uint8_t mon; //month
    switch (date[0]) {
        case 'J': mon = date[1] == 'a' ? 1 : date[2] == 'n' ? 6 : 7; break;
        case 'F': mon = 2; break;
        case 'A': mon = date[2] == 'r' ? 4 : 8; break;
        case 'M': mon = date[2] == 'r' ? 3 : 5; break;
        case 'S': mon = 9; break;
        case 'O': mon = 10; break;
        case 'N': mon = 11; break;
        case 'D': mon = 12; break;
    }
    uint8_t mday = conv2d(date + 4);
    uint8_t hour = conv2d(time);
    uint8_t min = conv2d(time + 3);
    uint8_t sec = conv2d(time + 6);
    uint8_t wday = 0;         // FIXME This is not properly initialized
	
	uint8_t TimeDate[7] = { sec, min, hour, wday, mday, mon, year_s };
